...
class CustomerTransactions(Resource):
    ...
    def get(self, customer_code):
        try:
            ...
            # Récupération du paramètre de période
            period = request.args.get('period', 'month')
            current_app.logger.info(f"Fetching transactions for period: {period}")
            ...
            try:
                transactions = MyWittiJetonsTransactions.query.filter(
                    ...
                    MyWittiJetonsTransactions.date_transaction <= now
                ).order_by(MyWittiJetonsTransactions.date_transaction.desc()).all()
            except Exception as e:
                current_app.logger.error(f"Error querying transactions: {str(e)}")
                transactions = []
            ...
            try:
                ...
                current_app.logger.error(f"Error formatting transactions: {str(e)}")
                transactions_list = []
            ...
            try:
                ...
                purchase_count = sum(1 for t in transactions_list if t['type'] == 'Purchase')
                reward_count = sum(1 for t in transactions_list if t['type'] == 'Reward')
                ...
                current_app.logger.error(f"Error calculating trends: {str(e)}")
                trends = {
...
class CustomerNotifications(Resource):
    ...
    def get(self, customer_code):
        try:
            ...
            # Récupération sécurisée des notifications de l'utilisateur
    ...
    def get(self, customer_code):
        try:
            ...
            try:
                ...
                current_app.logger.warning(f"Error accessing category: {str(e)}")
            ...
            try:
                ...
                current_app.logger.warning(f"Error calculating percentage: {str(e)}")
                percentage = 0
                tokens_to_next_tier = 0
